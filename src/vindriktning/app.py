...
from src.vindriktning.blocks.VindriktningInterface import VindriktningInterface
...
class App(Module):
    ...
    def __preinit__(self):
        ...
        fuse.trip_current.constrain_subset(L.Range.from_center(550 * P.mA, 10 * P.percent))
        ...
        # pm sensor
        self.vindriktning_interface.uart.connect(self.mcu.esp32_c3_mini_1.ic.esp32_c3.uart[1])
        ...
        self.vindriktning_interface.fan_enable.connect(self.mcu.esp32_c3_mini_1.ic.gpio[4])

        # pressence sensor
        self.pressence_sensor.uart.connect(self.mcu.uart)
        self.pressence_sensor.out.connect(self.mcu.esp32_c3_mini_1.ic.gpio[6])

        # I2C devices - connect individually
        i2c = self.mcu.esp32_c3_mini_1.ic.esp32_c3.i2c
        i2c.connect(self.co2_sensor.i2c, link=F.ElectricLogic.LinkIsolatedReference())
        i2c.connect(self.qwiic_connector.i2c, link=F.ElectricLogic.LinkIsolatedReference())
        i2c.connect(self.lux_sensor.ic.i2c, link=F.ElectricLogic.LinkIsolatedReference())

        # LEDs
        self.led_string.data_in.connect(
            self.mcu.esp32_c3_mini_1.ic.gpio[5],
        ...
        self.pressence_sensor.esphome_config.throttle.constrain_subset(default_update_interval)
        self.lux_sensor.ic.esphome_config.update_interval.constrain_subset(default_update_interval)
        self.led_string.max_refresh_rate.constrain_subset(60 * P.Hz)
        self.co2_sensor.esphome_config.update_interval.constrain_subset(default_update_interval)
...
class Vindriktning(Module):
    ...
    def __preinit__(self):
        self.app.vindriktning_interface.pm_sensor_connector.data.connect(self.pm1006.data)
        self.app.vindriktning_interface.pm_sensor_connector.power.connect(self.pm1006.power)
        self.app.vindriktning_interface.fan_controller.fan_connector.power.connect(self.fan.power)
