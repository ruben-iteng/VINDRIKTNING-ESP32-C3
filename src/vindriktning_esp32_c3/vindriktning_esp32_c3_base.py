    ...
    def __init__(self, pixels: int, buffered: bool = True) -> None:
        ...
        class _NODEs(Module.NODES()):
            ...
            for _ in range(pixels):
                decoupling_cap_led[_] = Capacitor(Constant(100 * n))
...
class Vindriktning_ESP32_C3(Module):
    def __init__(self) -> None:
        ...
        # easy ERC
        # for cmp in cmps:
        #   #for cmp_nodes in cmp.NODEs.get_all():
        #    if not isinstance(cmp, Module):
        #        continue
        #    for interface in cmp.IFs.get_all():
        #        #if interface is Electrical():
        #        if not interface.GIFs.connected.is_connected(# other connection):
        #            logger.warn(
        #                f"Electrical interface [{interface}] from component [{cmp}] is not connected!"
        #            )
