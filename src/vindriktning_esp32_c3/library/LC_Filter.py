...
class LC_Filter(Module):
    ...
    def __init__(self, inductance: Parameter, capacitance: Parameter):
        ...
        class _NODEs(Module.NODES()):
            ...
            capacitor = times(2, lambda: Capacitor(capacitance))

        self.NODEs = _NODEs(self)

        # Connections
        self.IFs.signal_in.connect_via(self.NODEs.inductor, self.IFs.signal_out)
        self.IFs.signal_in.connect_via(self.NODEs.capacitor[0], self.IFs.power.NODEs.lv)
        self.IFs.signal_out.connect_via(
            self.NODEs.capacitor[1], self.IFs.power.NODEs.lv
        )
